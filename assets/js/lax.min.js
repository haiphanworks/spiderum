...
var _loop=function _loop(_i2){var a=presets[_i2];var b=a.name.split(breakpointsSeparator);var breakpoint=b[1]?"".concat(breakpointsSeparator).concat(b[1]):'';a.value.split(" ").forEach(function(p){var bits=p.split("-");var fn=lax.presets[bits[0]];if(!fn){console.log("lax error: preset ".concat(bits[0]," is not defined"));}else{var d=fn(bits[1]);for(var k in d){el.setAttribute("".concat(k).concat(breakpoint),d[k]);}}});el.setAttribute("data-lax-anchor","self");el.attributes.removeNamedItem(a.name);};for(var _i2=0;_i2<presets.length;_i2++){_loop(_i2);}
